...
case class Expectation(option: AnyRef, value: Double) {
  def toLogoList = LogoList(option, Double.box(value))
}

class ChooserObject(val chooser: Chooser[AnyRef, AnyRef, Null]) extends ExtensionObject {
  ...
  def observe(choiceMade: AnyRef, resultObserved: AnyRef): Unit =
